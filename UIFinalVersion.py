...
class UI:
    ...
    def create_widgets(self):
        ...
        # Set default page
        self.on_nav_click("Upload", self.upload_screen)
    ...
    def dashboard_screen(self):
        ...
        # Recent Files Table - more compact size
        files_frame = tk.Frame(dashboard_frame, bg="white")
        files_frame.pack(pady=20, expand=False)  # Center horizontally, don't fill

        # Title for Recent Files
        title_frame = tk.Frame(files_frame, bg="white", width=400)
        title_frame.pack(pady=(0, 8), anchor="w")  # Align frame to the left
        tk.Label(title_frame, text="Recent Files", font=("Helvetica", 16, "bold"), bg="white").pack(side="left")  # Use side=left to prevent centering

        # Create a Treeview table with a vertical scrollbar for files - fixed width
        files_table = tk.Frame(files_frame, bg="white", width=400)
        files_table.pack(pady=5)

        tree = ttk.Treeview(files_table, columns=("name", "date"), show="headings", height=5)
        tree.heading("name", text="File Name", anchor="w")
        tree.heading("date", text="Time and Date", anchor="w")
        tree.column("name", width=350, anchor="w")  
        tree.column("date", width=250, anchor="w")  # Increased width to fit longer header

        vsb = ttk.Scrollbar(files_table, orient="vertical", command=tree.yview)
        tree.configure(yscrollcommand=vsb.set)

        tree.grid(row=0, column=0, sticky="nsew")
        vsb.grid(row=0, column=1, sticky="ns")
        files_table.grid_rowconfigure(0, weight=1)
        files_table.grid_columnconfigure(0, weight=1)

        # scan results/ for recent processed jsonl files
        script_dir = os.path.dirname(os.path.abspath(__file__))
        results_dir = os.path.join(script_dir, "results")
        history = []
        if os.path.isdir(results_dir):
            for fname in os.listdir(results_dir):
                if fname.endswith('.rules.v01.jsonl') or (fname.endswith('.jsonl') and fname != 'findings.jsonl'):
                    path = os.path.join(results_dir, fname)
                    try:
                        mtime = os.path.getmtime(path)
                    except Exception:
                        mtime = 0
                    history.append((fname, mtime))

        history.sort(key=lambda x: x[1], reverse=True)
        # show up to 20 most recent (tree will scroll if needed)
        for (fname, mtime) in history[:20]:
            dt = datetime.fromtimestamp(mtime).strftime('%H:%M %d/%m/%Y') if mtime else ''
            tree.insert("", "end", values=(fname, dt))
    ...
    def results_screen(self):
        ...
        def clear_all_filters():
            ...
            severity_var.set("All")  # Reset severity
            src_var.set("Any")  # Reset source IP
            dst_var.set("Any")  # Reset destination IP
            apply_filters()  # Re-apply filters to update the table
        
        tk.Button(controls, text="Clear", command=clear_all_filters).pack(side="left", padx=(0,8))
        ...
        tree.column("rule_id", width=80, anchor="center", stretch=False)
        tree.column("finding", width=300, anchor="w", stretch=False)
        tree.column("source", width=400, anchor="w", stretch=False)
        tree.column("destination", width=400, anchor="w", stretch=False)
        tree.column("protocol", width=100, anchor="center", stretch=False)
        tree.column("ports", width=140, anchor="center", stretch=False)
        tree.column("rationale", width=400, anchor="w", stretch=False)
        ...
        def normalize_addr(val):
            """Normalize address-like values into a comma-separated string for display and comparison."""
        ...
        def unique_ips(field):
            # try to gather common alternate keys too (e.g. src_addrs or raw.src)
            vals = set()
            for r in self.results_data:
                v = r.get(field) or r.get(field + "_addrs") or r.get("raw", {}).get(field, "")
                norm = normalize_addr(v)
                if norm:
                    vals.add(norm)
            vals = sorted(vals)
            return ["Any"] + vals
        ...
        def addr_matches(rec, field, selected):
            if selected == "Any":
                return True
            v = rec.get(field) or rec.get(field + "_addrs") or rec.get("raw", {}).get(field, "")
            norm = normalize_addr(v)
            if not norm:
                return False
            if selected == norm:
        ...
        def format_proto_ports(rec):
            """Return (proto_display, ports_display) similar to the tree formatter but compact for searching."""
            proto_set = []
            ports_tokens = []
            ...
            for svc in services:
                if isinstance(svc, str):
                    # some entries like 'tcp/80' or 'any'
                ...
                if "from" in svc and "to" in svc:
                    try:
                        ...
                        ports_tokens.append(clean(f"{svc.get('from')}-{svc.get('to')}"))
                    ...
                    if not ports:
                        ...
                        for p in ports:
                            if isinstance(p, dict) and "from" in p and "to" in p:
                                try:
                                    f = int(p.get("from")); t = int(p.get("to"))
                                    ports_tokens.append(str(f) if f == t else f"{f}-{t}")
                                except Exception:
                                    ports_tokens.append(clean(p))
                            else:
                                ports_tokens.append(clean(p))
                elif isinstance(ports, (int, str)):
                    ports_tokens.append(clean(ports))
                else:
                    ports_tokens.append("any")
        ...
        def filtered_data():
            ...
            def keep(r):
                ...
                # source/destination match using normalized addresses
                if s != "Any" and not addr_matches(r, "src", s):
                    return False
                if d != "Any" and not addr_matches(r, "dst", d):
                    return False

                # free-text search against all visible columns:
                # rule_id, finding (name/title), source, destination, protocol, ports, rationale, severity
                if q:
                    parts = []
                    parts.append(str(r.get("rule_id", "")))
                    parts.append(str(r.get("name") or r.get("title") or r.get("raw", {}).get("rule_id", "")))
                    src_norm = normalize_addr(r.get("src") or r.get("src_addrs") or r.get("raw", {}).get("src", ""))
                    dst_norm = normalize_addr(r.get("dst") or r.get("dst_addrs") or r.get("raw", {}).get("dst", ""))
                    parts.append(src_norm)
                    parts.append(dst_norm)
                    proto_display, ports_display = format_proto_ports(r)
                    parts.append(proto_display)
                    parts.append(ports_display)
                    parts.append(str(r.get("reason") or r.get("rationale") or r.get("raw", {}).get("reason", "")))
                    parts.append(str(r.get("severity", "")))
        ...
        # make filters reactive so selections apply immediately
        try:
            severity_menu.bind("<<ComboboxSelected>>", lambda e: apply_filters())
            src_menu.bind("<<ComboboxSelected>>", lambda e: apply_filters())
            dst_menu.bind("<<ComboboxSelected>>", lambda e: apply_filters())
        except Exception:
            # if widgets aren't in scope for some reason, ignore and continue
            pass

        # update on search box changes
        try:
            # trace_add is available on modern Tk; fall back to trace for older versions
            if hasattr(search_var, 'trace_add'):
                search_var.trace_add('write', lambda *args: apply_filters())
            else:
                search_var.trace('w', lambda *args: apply_filters())
        except Exception:
            pass
